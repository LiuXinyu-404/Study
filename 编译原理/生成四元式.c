...
//				 	ERROR('i',Line);
			...
			if(Symbol == COMMA || Symbol == SEMICN){
				ADDTable();
				if(SID.Type == INTTK){
					PrintMid("var", "int", "  ",SID.Name);
				}
				else if (SID.Type == CHARTK)
				{
					PrintMid("var", "char", "  ",SID.Name);
				}
				RLSymbols();
				VAR = 1;
				if(Symbol == SEMICN){
...
void Assign(){
	...
	else if(Symbol == LBRACK)
	{
		...
		EXPRS = 0;
		Expr();
		if(EXPRTYPE != INT && ITEMS == 1 && EXPRS == 1)
			ERROR('i',Line);
